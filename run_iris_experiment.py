...
def run_all_experiments(experiment_configs: List[Dict]):
    ...
    logger.info("Loading SMALL SUBSET of Iris data for testing...")
...
def plot_results(all_results: List[Dict]):
    ...
    for result in all_results:
        ...
        for round_data in result['round_history']:
            ...
            # Approx download = N_clients * bytes_per_client
            ...
            plot_data.append({
                 ...
                 # Add placeholder for quantum resources if tracked later
                 # 'Avg Gates': avg_gates_this_round,
                 # 'Avg Depth': avg_depth_this_round,
             })
    ...
    if not plot_data: logger.warning("No data available for plotting."); return
    ...
    fig1.tight_layout(rect=[0, 0, 0.85, 1]); save_plot(fig1, acc_dir, "accuracy_vs_round.png")
    ...
    if not plot_df_loss.empty:
        ...
        fig2.tight_layout(rect=[0, 0, 0.85, 1]); save_plot(fig2, loss_dir, "loss_vs_round.png")
    else: logger.info("Skipping loss plot (no valid data).")

    # 3. Accuracy vs. Cumulative Time
    fig3, ax3 = plt.subplots(figsize=(12, 6))
    sns.lineplot(data=plot_df, x='Cumulative Time (s)', y='Accuracy', hue='Experiment', marker='.', errorbar=None, ax=ax3)
    ax3.set_title('Accuracy vs. Wall Clock Time'); ax3.set_ylabel('Global Model Accuracy')
    ax3.legend(title='Experiment', bbox_to_anchor=(1.05, 1), loc='upper left')
    fig3.tight_layout(rect=[0, 0, 0.85, 1]); save_plot(fig3, res_dir, "accuracy_vs_time.png")

    # 4. Accuracy vs. Cumulative Communication (Upload)
    fig4, ax4 = plt.subplots(figsize=(12, 6))
    sns.lineplot(data=plot_df, x='Cumulative Comm Upload (MB)', y='Accuracy', hue='Experiment', marker='.', errorbar=None, ax=ax4)
    ax4.set_title('Accuracy vs. Communication (Upload)'); ax4.set_ylabel('Global Model Accuracy')
    ax4.legend(title='Experiment', bbox_to_anchor=(1.05, 1), loc='upper left')
    fig4.tight_layout(rect=[0, 0, 0.85, 1]); save_plot(fig4, res_dir, "accuracy_vs_comm_upload.png")

    # 5. Final Accuracy Comparison Bar Chart
    fig5, ax5 = plt.subplots(figsize=(10, 7))
    sns.barplot(data=final_metrics_df, x='Final Accuracy', y='Experiment', ax=ax5, orient='h')
    ax5.set_title('Comparison of Final Accuracy')
    ax5.set_xlabel('Accuracy'); ax5.set_ylabel(''); ax5.set_xlim(0, 1.05)
    fig5.tight_layout(); save_plot(fig5, comp_dir, "final_accuracy_comparison.png")

    # 6. Total Time Comparison Bar Chart
    fig6, ax6 = plt.subplots(figsize=(10, 7))
    sns.barplot(data=final_metrics_df, x='Total Time (s)', y='Experiment', ax=ax6, orient='h')
    ax6.set_title('Comparison of Total Training Time')
    ax6.set_xlabel('Time (seconds)'); ax6.set_ylabel('')
    fig6.tight_layout(); save_plot(fig6, comp_dir, "total_time_comparison.png")

    # 7. Total Communication Comparison Bar Chart
    fig7, ax7 = plt.subplots(figsize=(10, 7))
    sns.barplot(data=final_metrics_df, x='Total Comm Upload (MB)', y='Experiment', ax=ax7, orient='h')
    ax7.set_title('Comparison of Total Communication Upload')
    ax7.set_xlabel('Megabytes (MB)'); ax7.set_ylabel('')
    fig7.tight_layout(); save_plot(fig7, comp_dir, "total_comm_comparison.png")
...
EXPERIMENTS = [
    ...
    {
        "name": "Iris_MLP_FedAvg_NonIID_a0.3", # More specific name
        "model": {"type": "classical_mlp", "hidden_dim": 32},
    ...
    {
        "name": "Iris_MLP_FedProx_IID",
        "model": {"type": "classical_mlp", "hidden_dim": 32},
        "data_partition": {"iid": True},
        "aggregation_overrides": {"strategy": "FedProx"}, # Strategy name signals intent
        "client_overrides": {
            "client": {"local_epochs": 5},
            "optimization": {"learning_rate": 0.02, "optimizer": {"name": "Adam"}},
            "regularization": {"proximal_mu": 0.01} # Activate FedProx term
        },
        "server_overrides": {"server": {"num_rounds": 20}, "client_management": {"num_clients": 5}}
    },
    {
        "name": "Iris_MLP_FedDive_IID",
        "model": {"type": "classical_mlp", "hidden_dim": 32},
        "data_partition": {"iid": True},
        "aggregation_overrides": {
             "strategy": "FedDive",
             "strategies": { "FedDive": {"params": {"momentum": 0.9, "temperature": 1.0, "normalize_distances": True}}} # Pass params
             },
        "client_overrides": {"client": {"local_epochs": 5}, "optimization": {"learning_rate": 0.02, "optimizer": {"name": "Adam"}}},
        "server_overrides": {"server": {"num_rounds": 20}, "client_management": {"num_clients": 5}}
    },
     {
        "name": "Iris_MLP_FedDive_NonIID_a0.3",
        "model": {"type": "classical_mlp", "hidden_dim": 32},
        "data_partition": {"iid": False, "alpha": 0.3},
        "aggregation_overrides": {
             "strategy": "FedDive",
             "strategies": { "FedDive": {"params": {"momentum": 0.9, "temperature": 1.0, "normalize_distances": True}}}
             },
        "client_overrides": {"client": {"local_epochs": 5}, "optimization": {"learning_rate": 0.02, "optimizer": {"name": "Adam"}}},
        "server_overrides": {"server": {"num_rounds": 20}, "client_management": {"num_clients": 5}}
    },

    # === VQC Experiments (Using PCA=3 features -> 3 qubits) ===
    # Using quantum_features implicitly sets n_qubits for VQC create_model
    {
        "name": "Iris_VQC_FedAvg_IID_PCA3",
        "model": {"type": "vqc", "quantum_features": 3, "n_layers": 2},
        "data_partition": {"iid": True},
        "aggregation_overrides": {"strategy": "FedAvg"}, # Base strategy for QuantumAgg wrapper
    ...
    {
        "name": "Iris_VQC_FedAvg_NonIID_a0.3",
        "model": {"type": "vqc", "quantum_features": 3, "n_layers": 2},
        "data_partition": {"iid": False, "alpha": 0.3},
        "aggregation_overrides": {"strategy": "FedAvg"},
        "client_overrides": {"client": {"local_epochs": 2}, "optimization": {"learning_rate": 0.1}},
     ...
     {
        "name": "Iris_VQC_FedDive_IID_PCA3",
        "model": {"type": "vqc", "quantum_features": 3, "n_layers": 2},
        "data_partition": {"iid": True},
        "aggregation_overrides": {
             "strategy": "FedDive",
             "strategies": {"FedDive": {"params": {"momentum": 0.5, "temperature": 0.5}}} # Tune FedDive params
             },
        "client_overrides": {"client": {"local_epochs": 2}, "optimization": {"learning_rate": 0.1}},
        "server_overrides": {"server": {"num_rounds": 15}, "client_management": {"num_clients": 5}}
    },
     {
        "name": "Iris_VQC_FedProx_IID_PCA3",
        "model": {"type": "vqc", "quantum_features": 3, "n_layers": 2},
        "data_partition": {"iid": True},
    ...
    # === QNN/Hybrid Experiments (Using PCA=3 features -> 3 qubits) ===
    {
        "name": "Iris_Hybrid_FedAvg_IID_PCA3",
        "model": {"type": "hybrid", "quantum_features": 3, "n_qubits": 3, "n_layers": 1, "classical_hidden_dim": 8},
        "data_partition": {"iid": True},
        "aggregation_overrides": {"strategy": "FedAvg"}, # Use base FedAvg directly
        "client_overrides": {"client": {"local_epochs": 3}, "optimization": {"learning_rate": 0.01, "optimizer": {"name": "Adam"}}},
        "server_overrides": {"server": {"num_rounds": 15}, "client_management": {"num_clients": 5}}
    },
     {
        "name": "Iris_Hybrid_FedDive_IID_PCA3",
        "model": {"type": "hybrid", "quantum_features": 3, "n_qubits": 3, "n_layers": 1, "classical_hidden_dim": 8},
        "data_partition": {"iid": True},
        "aggregation_overrides": {
             "strategy": "FedDive",
             "strategies": {"FedDive": {"params": {"momentum": 0.9, "temperature": 1.0}}}
             }, # Use FedDive directly
        "client_overrides": {"client": {"local_epochs": 3}, "optimization": {"learning_rate": 0.01, "optimizer": {"name": "Adam"}}},
        "server_overrides": {"server": {"num_rounds": 15}, "client_management": {"num_clients": 5}}
    },
    # Add QNN experiments if desired
    {
        "name": "Iris_QNN_FedAvg_IID_PCA3",
        "model": {"type": "qnn", "quantum_features": 3, "n_qubits": 3, "n_layers": 2},
        "data_partition": {"iid": True},
        "aggregation_overrides": {"strategy": "FedAvg"},
        "client_overrides": {"client": {"local_epochs": 3}, "optimization": {"learning_rate": 0.01, "optimizer": {"name": "Adam"}}},
        "server_overrides": {"server": {"num_rounds": 15}, "client_management": {"num_clients": 5}}
