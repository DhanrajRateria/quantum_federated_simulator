...
from src.utils.data_utils import (set_seed, load_and_preprocess_data, load_multi_modal_synthetic_data, pathological_synthetic_partition)
...
from src.visualization.integrate_visualizations import enhanced_plot_bc_results as plot_results
...
def setup_federated_scenario(exp_config: Dict):
    ...
    if dataset_name == "synthetic_multi_modal":
        train_data, test_data, n_features, n_classes = load_multi_modal_synthetic_data(random_state=exp_config.get("seed", 42))
    ...
    # --- Data Partitioning ---
    partition_type = exp_config.get("data_partition", {}).get("type")
    if partition_type == "pathological_synthetic": # New partition type
        num_clients = server_cfg.get("client_management", {}).get("num_clients", 5)
        client_datasets = pathological_synthetic_partition(train_data, num_clients)
    else:
        num_clients = server_cfg.get("client_management", {}).get("num_clients", 5)
...
EXPERIMENTS_TO_RUN = [
   {
    "name": "Synthetic_MultiModal_FedAvg",
    "model": {"type": "classical_mlp", "hidden_dim": 32}, # A small MLP is sufficient
    "data_config": {"name": "synthetic_multi_modal"},
    "data_partition": {"type": "pathological_synthetic"},
    "aggregation_overrides": {"type": "fedavg"},
    "client_overrides": {
        "client": {"local_epochs": 15}, # Train well to find the local mode
        "optimization": {"learning_rate": 0.01}
    },
    "server_overrides": {"server": {"num_rounds": 40}, "client_management": {"num_clients": 10}}
},
{
    "name": "Synthetic_MultiModal_FedProx",
    ...
    "client_overrides": {
        ...
        "optimization": {"learning_rate": 0.01},
        "regularization": {"proximal_mu": 0.1} # Higher mu to fight divergence
    },
    "server_overrides": {"server": {"num_rounds": 40}, "client_management": {"num_clients": 10}}
},
{
    "name": "Synthetic_MultiModal_FedDive",
    "model": {"type": "classical_mlp", "hidden_dim": 32},
    "data_config": {"name": "synthetic_multi_modal"},
    "data_partition": {"type": "pathological_synthetic"},
    "aggregation_overrides": {"type": "feddive", "momentum": 0.5, "temperature": 0.5}, # Lower temp to reward diversity
    "client_overrides": {
        "client": {"local_epochs": 15},
        "optimization": {"learning_rate": 0.01}
    },
